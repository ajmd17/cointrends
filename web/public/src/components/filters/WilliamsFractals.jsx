...
class WilliamsFractals extends React.Component {
  static propTypes = {
    fractals: React.PropTypes.shape({
      up: React.PropTypes.arrayOf(React.PropTypes.object),
      down: React.PropTypes.arrayOf(React.PropTypes.object)
    }).isRequired
  };

  render() {
    const { fractals } = this.props;

    return (
      <div>
        {Object.keys(fractals).map((key) => fractals[key].map((fractal, i) => {
          if (fractal == null) return null;
          return (
            <Arrow direction={key} object={fractal} key={i} />
          );
        }))}
      </div>
    );
